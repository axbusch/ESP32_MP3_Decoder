...
typedef enum {
	CONTAINER_RAW = 1, CONTAINER_MP4 = 2
} container_format_t;
...
void libfaac_decoder_task(void *pvParameters) {
	...
	buffer_t buf = { .len = FAAD_BYTE_BUFFER_SIZE };
	...
	if (content_type == AUDIO_MP4) {
		err = NeAACDecInit2(decoder, demux_res.codecdata,
				demux_res.codecdata_len, &samp_rate, &chan);
	} else {
		err = NeAACDecInit(decoder, demux_res.codecdata,
				demux_res.codecdata_len, &samp_rate, &chan);
	}
	...
	pcm_format_t pcm_fmt = { .sample_rate = samp_rate, .bit_depth =
			I2S_BITS_PER_SAMPLE_16BIT, .num_channels = chan, .buffer_format =
			PCM_INTERLEAVED };
