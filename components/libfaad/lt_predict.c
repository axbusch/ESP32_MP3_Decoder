...
ALIGN static const real_t codebook[8] = { REAL_CONST(0.570829), REAL_CONST(
		0.696616), REAL_CONST(0.813004), REAL_CONST(0.911304), REAL_CONST(
		0.984900), REAL_CONST(1.067894), REAL_CONST(1.194601), REAL_CONST(
		1.369533) };
	...
	if (ics->window_sequence != EIGHT_SHORT_SEQUENCE) {
		if (ltp->data_present) {
			...
			for (i = 0; i < num_samples; i++) {
				 ...
				 */
				x_est[i] = (real_t) lt_pred_stat[num_samples + i - ltp->lag]
						* codebook[ltp->coef];
#endif
			}
			...
			for (sfb = 0; sfb < ltp->last_band; sfb++) {
				if (ltp->long_used[sfb]) {
					...
					uint16_t high = min(ics->swb_offset[sfb + 1],
							ics->swb_offset_max);
