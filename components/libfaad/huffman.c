...
hcb_2_quad *hcb_2_quad_table[] = { 0, hcb1_2, hcb2_2, 0, hcb4_2, 0, 0, 0, 0, 0,
		0, 0 };

hcb_2_pair *hcb_2_pair_table[] = { 0, 0, 0, 0, 0, 0, hcb6_2, 0, hcb8_2, 0,
		hcb10_2, hcb11_2 };
...
static uint8_t huffman_binary_quad(uint8_t cb, bitfile *ld, int16_t *sp) {
	...
	while (!hcb3[offset].is_leaf) {
		uint8_t b = faad_get1bit(
				ld DEBUGVAR(1,255,"huffman_spectral_data():3"));
...
static uint8_t huffman_binary_pair(uint8_t cb, bitfile *ld, int16_t *sp) {
	...
	while (!hcb_bin_table[cb][offset].is_leaf) {
		uint8_t b = faad_get1bit(
				ld DEBUGVAR(1,255,"huffman_spectral_data():9"));
...
static void vcb11_check_LAV(uint8_t cb, int16_t *sp) {
	static const uint16_t vcb11_LAV_tab[] = { 16, 31, 47, 63, 95, 127, 159, 191,
