		...
		for (b = 0; b < icsr->window_group_length[g]; b++) {
			for (sfb = 0; sfb < icsr->max_sfb; sfb++) {
				if (is_intensity(icsr, g, sfb)) {
...
#ifndef FIXED_POINT
					scale = (real_t) pow(0.5,
							(0.25 * icsr->scale_factors[g][sfb]));
#else
					...
					for (i = icsr->swb_offset[sfb];
							i
									< min(icsr->swb_offset[sfb + 1],
											ics->swb_offset_max); i++) {
#ifndef FIXED_POINT
						r_spec[(group * nshort) + i] = MUL_R(
								l_spec[(group * nshort) + i], scale);
#else
                        if (exp < 0)
						...
						if (is_intensity(icsr, g, sfb)
							...
							r_spec[(group * nshort) + i] = -r_spec[(group
									* nshort) + i];
					}
				}
			}
