...
static void init_i2s(renderer_config_t *config) {
	...
	if (out_info.revision > 0
			&& (config->output_mode == I2S || config->output_mode == I2S_MERUS)) {
		use_apll = 1;
		ESP_LOGI(TAG, "chip revision %d, enabling APLL for I2S",
				out_info.revision);
	}
	 ...
	 */
	i2s_config_t i2s_config = { .mode = mode,          // Only TX
			.sample_rate = config->sample_rate, .bits_per_sample =
					config->bit_depth, .channel_format =
					I2S_CHANNEL_FMT_RIGHT_LEFT,   // 2-channels
			.communication_format = comm_fmt, .dma_buf_count = 32, // number of buffers, 128 max.
			.dma_buf_len = 64,                          // size of each buffer
			.intr_alloc_flags = ESP_INTR_FLAG_LEVEL1,       // Interrupt level 1
			.use_apll = use_apll };

	i2s_pin_config_t pin_config = { .bck_io_num = GPIO_NUM_26, .ws_io_num =
			GPIO_NUM_25, .data_out_num = GPIO_NUM_22, .data_in_num =
			I2S_PIN_NO_CHANGE };
...
void render_samples(char *buf, uint32_t buf_len, pcm_format_t *buf_desc) {
	...
	if (buf_desc->bit_depth == renderer_instance->bit_depth
		...
		while (bytes_left > 0 && renderer_status != STOPPED) {
			i2s_write(renderer_instance->i2s_num, (const void*) buf, bytes_left,
					&bytes_written, max_wait);
	...
	for (int i = 0; i < num_samples; i++) {
		...
		if (renderer_instance->output_mode == DAC_BUILT_IN) {
			...
			i2s_write(renderer_instance->i2s_num, (const void*) &sample,
					sizeof(sample), &bytes_pushed, max_wait);
		} else {
			...
			case I2S_BITS_PER_SAMPLE_16BIT:
				...
				/* low - high / low - high */
				const char samp32[4] =
						{ ptr_l[0], ptr_l[1], ptr_r[0], ptr_r[1] };
				i2s_write(renderer_instance->i2s_num, (const void*) &samp32,
						sizeof(samp32), &bytes_pushed, max_wait);
				break;

			case I2S_BITS_PER_SAMPLE_32BIT:
				; // workaround

				const char samp64[8] = { 0, 0, ptr_l[0], ptr_l[1], 0, 0,
						ptr_r[0], ptr_r[1] };
				i2s_write(renderer_instance->i2s_num, (const void*) &samp64,
						sizeof(samp64), &bytes_pushed, max_wait);
				break;

			default:
				ESP_LOGE(TAG, "bit depth unsupported: %d",
						renderer_instance->bit_depth);
			}
		}
...
void renderer_init(renderer_config_t *config) {
	...
	ESP_LOGI(TAG, "init I2S mode %d, port %d, %d bit, %d Hz",
			config->output_mode, config->i2s_num, config->bit_depth,
			config->sample_rate);
	init_i2s(config);
