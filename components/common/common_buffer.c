...
size_t fill_read_buffer(buffer_t *buf) {
	...
	size_t bytes_to_read = min(buf_free_capacity_after_purge(buf),
			spiRamFifoFill());
...
int buf_seek_abs(buffer_t *buf, uint32_t pos) {
	...
	if (pos > buf->write_pos) {
		ESP_LOGE(TAG, "buf_seek_abs failed, pos = %u larger than fill_pos %u",
...
size_t buf_read(void *ptr, size_t size, size_t count, buffer_t *buf) {
	...
	if (bytes_to_copy > buf->len) {
		ESP_LOGE(TAG,
				"buf_read failed, bytes_to_copy = %d larger than buffer size %d",
	...
	if (bytes_to_copy > buf_data_unread(buf)) {
		ESP_LOGE(TAG, "buf_read failed bytes_to_copy %d, buf_data_unread %d",
