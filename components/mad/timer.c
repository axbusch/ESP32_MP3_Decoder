...
signed long mad_timer_count(mad_timer_t timer, enum mad_units units) {
	...
	case MAD_UNITS_75_FPS:
		return timer.seconds * (signed long) units
				+ (signed long) scale_rational(timer.fraction,
						MAD_TIMER_RESOLUTION, units);
...
unsigned long mad_timer_fraction(mad_timer_t timer, unsigned long denom) {
	...
	case 0:
		...
		MAD_TIMER_RESOLUTION / timer.fraction :
...
void mad_timer_string(mad_timer_t timer, char *dest, char const *format,
		enum mad_units units, enum mad_units fracunits, unsigned long subparts) {
	...
	case MAD_UNITS_MINUTES:
		...
		sprintf(dest, format, minutes, (unsigned int) (seconds % 60), frac,
				sub);
		break;
