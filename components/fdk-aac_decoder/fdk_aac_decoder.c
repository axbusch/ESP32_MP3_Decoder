...
void fdkaac_decoder_task(void *pvParameters) {
	...
	ESP_LOGI(TAG, "(line %u) free heap: %u", __LINE__,
	...
	while (!player->media_stream->eof) {
		...
		if (first_frame) {
			...
			ESP_LOGI(TAG,
					"pcm_size %d, channels: %d, sample rate: %d, object type: %d, bitrate: %d",
					pcm_size, mStreamInfo->numChannels, mStreamInfo->sampleRate,
					mStreamInfo->aot, mStreamInfo->bitRate);
